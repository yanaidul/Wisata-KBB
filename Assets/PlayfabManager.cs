...
public class PlayfabManager : BackendHandler
{
    ...
    [Header("Sorted User List")]
    [SerializeField] private List<PlayerDataScriptableObject.PlayerDataStructure> _sortedUsernameList = new List<PlayerDataScriptableObject.PlayerDataStructure>();
    ...
    private void GetPlayerData()
    {
        ...
        StartCoroutine(WaitForAllUserLoaded());
    ...
    private void SortPlayfabIDByUsername()
    {
        _sortedUsernameList.Sort((a, b) => a.username[0].CompareTo(b.username[0]));

        foreach (var item in _sortedUsernameList)
        {
            GameObject userContainer = Instantiate(_userPrefab, _container.transform);
            int _totalProgression = (item.sejarahProgression + item.pengertianProgression +
                                    item.wisataAlamProgression + item.wisataBuatanProgression +
    ...
    private IEnumerator WaitForAllUserLoaded()
    {
        while (_playFabIdList.Count != _sortedUsernameList.Count)
        {
            Debug.Log("Wait User Loaded On Progress");

            yield return new WaitForSeconds(2f);
        }

        SortPlayfabIDByUsername();
    }
